...
export default function SidePanel() {
  const { session } = useContext(SessionContext) as SessionContextProps;
      ...
      <div className="mt-8 flex flex-col items-center w-60">
          ...
          <div className="px-2 py-2 bg-zinc-100 rounded-full">
            {(session && session.profilePicture && (
              <img
                ...
                className="rounded-full bg-gray-300 w-40 h-40"
              />
            )) || (
              <div className="rounded-full bg-gray-300 w-40 h-40 flex justify-center items-center">
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-zinc-500">
            {(session && `@${session.handle}`) || "Viewing as a guest"}
          </p>
        </div>
